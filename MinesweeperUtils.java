 ...
 * and getCol() methods.
 * 
 * **Edited getAdjacent method to be public to be used to find # of mines**
 * 
...
public class MinesweeperUtils {
     ...
     *         array
     */
    public static <T extends MineTile> ArrayList<T> getAdjacent(int x, int y, T[][] board) {
    ...
    public static <T extends MineTile> ArrayList<T> expandTile(T tile, T[][] board) {
        ...
        while (!stack.empty()) {
            ...
            visited.add(current); // keep track of visited tiles (we don't want
                                    // an infinite loop!)
            if (current.getAdjacent() == 0) {//Tile getAdj method
                for (T b : getAdjacent(current.getRow(), current.getCol(), board)) {//gets 8 tiles
